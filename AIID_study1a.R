...
data=read.csv("/Users/gabriellechristinemartin/Desktop/AIID Guns Study 1ab data/Study 1a data/UFGunJamesVersion.csv", na.strings = ".", stringsAsFactors = FALSE)
...
data2 <- data %>%
  select(Csupp, C1, C2, C3, C4, R1W, R2B, R3H, R4A, R5N, Why1n, Why2n, Why3n, Politic, sex, age, Educ, FSS) %>%
  mutate(sex = as.factor(sex)) %>%
  mutate(Educ = as.factor(Educ)) %>%
  mutate(Resid = as.factor(FSS))
...
table(data3$sex)
levels(data3$sex) <- c("Male", "Female")
summary(data3$sex)
...
#creating the gun-owner groups contrast codes to serve as predictors in our regression models
...
write.csv(data3,"/Users/gabriellechristinemartin/Desktop/AIID Guns Study 1ab data/Study 1a data/UFGunJamesVersion_15pctsample.csv", row.names = TRUE)

# read in the 15% sample data to work with

data4=read.csv("/Users/gabriellechristinemartin/Desktop/AIID Guns Study 1ab data/Study 1a data/UFGunJamesVersion_15pctsample.csv")
...
table(data5$blackvnot) # n = 72 Black
table(data5$Code1) # we only have 5 Black gun owners in the 15% sample; versus 67 non-owners
table(data5$Code2) # we have 1 Black recreation owner and 4 protection-owners
